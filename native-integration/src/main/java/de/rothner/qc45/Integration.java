...
public final class Integration {
    ...
    public static Integration start() throws Exception {
        ...
        if (loadManagerEnabled || failbackEnabled) {
            ...
            meter = new KsemClient(ksemHost, ksemPort, ksemUnit, ksemTimeoutMs, ksemScale, legacyLowWord);
        }
        ...
        if (loadManagerEnabled) {
            loadManager = new LoadManager(station, meter,
                decimal(p, "loadmanager.targetA", 32.0d), loadManagerGuardA,
                decimal(p, "loadmanager.hysteresisA", 0.8d),
                integer(p, "loadmanager.minDcKw", 5), integer(p, "loadmanager.maxDcKw", 50),
                integer(p, "loadmanager.minAcKw", 5), integer(p, "loadmanager.maxAcKw", 22),
                integer(p, "loadmanager.rampUpKwPerLoop", 2), integer(p, "loadmanager.intervalMs", 1000));
        }

        GridFailback failback = null;
        if (failbackEnabled) {
            failback = new GridFailback(station, meter, failbackReduceA,
                integer(p, "failback.reduceDelayMs", 500), decimal(p, "failback.tripA", 35.0d),
                integer(p, "failback.tripDelayMs", 250), decimal(p, "failback.instantTripA", 38.0d),
                integer(p, "failback.reduceDcKw", 5), integer(p, "failback.reduceAcKw", 5),
                integer(p, "failback.intervalMs", 200), bool(p, "failback.tripOnMeterFailure", true),
                integer(p, "failback.meterFailureMs", 3000));
        }

        Integration integration = new Integration(station, modbus, ocppBridge, ocpp15Bridge, loadManager, failback);
    ...
    private static Properties loadProperties() throws Exception {
        ...
        File file = explicit == null || explicit.trim().length() == 0
            ? new File("/home/mobie/evcsd/qc45-integration.properties") : new File(explicit.trim());
