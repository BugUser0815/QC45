...
public final class OcppBridgeClient extends Thread {
    ...
    public OcppBridgeClient(ReflectionQC45 station, String url, String username, String password,
                            String caFile, boolean insecure) throws Exception {
    ...
    public void rememberTransaction(int transactionId, int connector) {
        synchronized (txConnector) { txConnector.put(Integer.valueOf(transactionId), Integer.valueOf(connector)); }
    }
    ...
    private void onMessage(String msg) throws Exception {
        ...
        System.out.println("[QC45] OCPP bridge backend CALL " + action + " " + payload);
        ...
        if ("RemoteStopTransaction".equals(action)) {
            ...
            Integer connector;
            synchronized (txConnector) { connector = txConnector.get(Integer.valueOf(tx)); }
            if (connector != null) {
                try { station.remoteStop(connector.intValue()); sendResult(uid, "{\"status\":\"Accepted\"}"); }
    ...
    private static final class WebSocket {
        ...
        void connect() throws Exception {
            ...
            if(!insecure){ try { SSLParameters p=socket.getSSLParameters(); p.setEndpointIdentificationAlgorithm("HTTPS"); socket.setSSLParameters(p); } catch(Throwable ignored){} }
            ...
            String accept=header(h,"Sec-WebSocket-Accept");String exp=base64(MessageDigest.getInstance("SHA-1").digest((key+"258EAFA5-E914-47DA-95CA-C5AB0DC85B11").getBytes(UTF8)));if(!exp.equals(accept))throw new IOException("Invalid Sec-WebSocket-Accept");open=true;
        }
        synchronized void sendText(String s)throws IOException{sendFrame(1,s.getBytes(UTF8));}
        String readText(int timeout)throws IOException{if(!open)return null;socket.setSoTimeout(timeout);try{while(open){Frame f=readFrame();if(f.op==1)return new String(f.data,UTF8);if(f.op==8){close();return null;}if(f.op==9)sendFrame(10,f.data);}}catch(java.net.SocketTimeoutException e){return null;}return null;}
        ...
        private Frame readFrame()throws IOException{int a=in.read(),b=in.read();if(a<0||b<0)throw new EOFException();int op=a&15;boolean masked=(b&128)!=0;long n=b&127;if(n==126)n=((long)readByte()<<8)|readByte();else if(n==127){n=0;for(int i=0;i<8;i++)n=(n<<8)|readByte();}if(n>1024*1024)throw new IOException("frame too large");byte[] m=null;if(masked){m=new byte[4];readFully(in,m);}byte[] d=new byte[(int)n];readFully(in,d);if(m!=null)for(int i=0;i<d.length;i++)d[i]^=m[i&3];return new Frame(op,d);}
        ...
        private static final class Frame{final int op;final byte[] data;Frame(int o,byte[]d){op=o;data=d;}}
    }
}
